...
@section('title', __('educational::messages.add_schedule'))
        ...
        <div class="col-xl-10">
            <div class="card border-0 shadow-sm overflow-hidden">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <h5 class="card-title mb-0 fw-bold">
                                {{ __('educational::messages.add_schedule') ?? 'إضافة جدول تدريبي' }}</h5>
                            <p class="text-muted mb-0 small">تحديد توقيتات المحاضرات المتكررة وربطها بالمجموعة والقاعة.</p>
                        </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('educational.schedules.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-0">
                                    ...
                                    <select name="program_id" id="program_id"
                                        ...
                                        @php $reqProgramId = request('program_id'); @endphp
                                        @foreach ($programs as $program)
                                            <option value="{{ $program->id }}"
                                                {{ old('program_id') == $program->id || $reqProgramId == $program->id ? 'selected' : '' }}>
                                                {{ $program->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('program_id')
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-0">
                                    ...
                                    <select name="group_id" id="group_id"
                                        class="form-select py-2 @error('group_id') is-invalid @enderror" required disabled>
                                        <option value="">
                                            {{ __('educational::messages.select_program_first') ?? 'يرجى اختيار البرنامج أولاً' }}
                                        </option>
                                    </select>
                                    @error('group_id')
                                        <div class="text-danger small mt-1">{{ $message }}</div>
                            ...
                            <div class="col-12">
                                <div class="card bg-light border-0 shadow-none mb-0">
                                    <div class="card-body p-3">
                                        <h6 class="fw-bold mb-3 text-dark"><i class="ri-user-star-line me-2 text-info"></i>
                                            بيانات المُدرب والدرس</h6>
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                <label
                                                ...
                                                <select name="instructor_profile_id"
                                                    ...
                                                    @foreach ($instructors as $instructor)
                                                        <option value="{{ $instructor->id }}"
                                                            {{ old('instructor_profile_id') == $instructor->id ? 'selected' : '' }}>
                                                ...
                                                <select name="session_type_id"
                                                    ...
                                                    @foreach ($sessionTypes as $type)
                                                        <option value="{{ $type->id }}"
                                                            {{ old('session_type_id') == $type->id ? 'selected' : '' }}>
                                                            {{ $type->name }}</option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select name="room_id"
                                                    ...
                                                    @foreach ($rooms as $room)
                                                        <option value="{{ $room->id }}"
                                                            ...
                                                            {{ $room->name }}
                                                            ({{ $room->floor->building->campus->name ?? '' }})
                                                        </option>
                                                    @endforeach
                                                </select>
                                                @error('room_id')
                            ...
                            <div class="col-12">
                                <div class="card bg-light border-0 shadow-none mb-0">
                                    <div class="card-body p-3">
                                        <h6 class="fw-bold mb-3 text-dark"><i class="ri-time-line me-2 text-info"></i> توقيت
                                            ومعدل التكرار</h6>
                                        <div class="row g-3">
                                            <div class="col-md-4">
                                                <label
                                                ...
                                                <select name="recurrence_type"
                                                    ...
                                                    required>
                                                    @php $reqRecurrence = request('recurrence_type', 'weekly'); @endphp
                                                    <option value="weekly"
                                                        {{ old('recurrence_type', $reqRecurrence) == 'weekly' ? 'selected' : '' }}>
                                                        {{ __('educational::messages.weekly') ?? 'أسبوعيا' }}</option>
                                                    <option value="biweekly_even"
                                                        {{ old('recurrence_type', $reqRecurrence) == 'biweekly_even' ? 'selected' : '' }}>
                                                        {{ __('educational::messages.biweekly_even') ?? 'كل أسبوعين (زوجي)' }}
                                                    </option>
                                                    <option value="biweekly_odd"
                                                        {{ old('recurrence_type', $reqRecurrence) == 'biweekly_odd' ? 'selected' : '' }}>
                            ...
                            <div class="col-12">
                                <div class="card bg-light border-0 shadow-none mb-0">
                                    <div class="card-body p-3">
                                        <h6 class="fw-bold mb-1 text-dark"><i class="ri-survey-line me-2 text-info"></i>
                                            التقييم التلقائي <small class="text-muted fw-normal">(اختياري)</small></h6>
                                        <p class="text-muted small mb-3"><i class="ri-information-line me-1"></i> سيتم
                                            تعيين هذا النموذج تلقائياً لكل محاضرة يتم إنشاؤها من هذا الجدول.</p>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <select name="evaluation_form_id"
                                                    ...
                                                    @foreach ($evaluationForms as $form)
                                                        <option value="{{ $form->id }}"
                                                            {{ old('evaluation_form_id') == $form->id ? 'selected' : '' }}>
                                                            {{ $form->title }}
                            ...
                            <div class="col-md-12">
                                <div class="d-flex align-items-center gap-3 p-3 bg-light rounded">
                                    ...
                                    <small class="text-muted">سيتم توليد المحاضرات المتكررة بناءً على هذا الجدول فقط عند
                                        التفعيل.</small>
                                </div>
                            </div>
                            ...
                            <div class="col-12 border-top pt-4">
                                <div class="hstack gap-2 justify-content-end">
                                    ...
                                    <button type="submit" class="btn btn-info px-5 shadow-sm fw-bold">
                                        ...
                                        {{ __('educational::messages.save') ?? 'حفظ' }}
                                    </button>
                                </div>
                            </div>
                        </div>
...
@push('scripts')
    <script>
        function loadGroups(programId, selectedGroupId = null) {
            const groupSelect = document.getElementById('group_id');
            groupSelect.innerHTML = '<option value="">جارٍ التحميل...</option>';
            groupSelect.disabled = true;

            if (!programId) {
                groupSelect.innerHTML = '<option value="">يرجى اختيار البرنامج أولاً</option>';
                return;
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const reqGroupId = '{{ request('group_id') ?? old('group_id') }}';
            if (progId) {
                loadGroups(progId, reqGroupId);
            }
        });
    </script>
@endpush
