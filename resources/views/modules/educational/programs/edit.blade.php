...
@section('title', __('educational::messages.edit_program'))
...
<div class="row justify-content-center mt-4">
    <div class="col-xl-9">
        <form action="{{ route('educational.programs.update', $program->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="card-header bg-soft-warning border-0 py-3">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <h5 class="card-title mb-0 fw-bold">{{ __('educational::messages.edit_program') }}: <span class="text-primary">{{ $program->name }}</span></h5>
                            <p class="text-muted mb-0 small">تحديث بيانات البرنامج التدريبي والفروع المسؤولة عنه.</p>
                        </div>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-12">
                            <div class="d-flex align-items-center mb-1">
                                ...
                                <h6 class="mb-0 fw-bold border-bottom border-warning border-2 pb-1">المعلومات الأساسية</h6>
                            </div>
                        </div>
                        ...
                        <div class="col-md-8">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="name" class="form-control border-left-0 py-2 @error('name') is-invalid @enderror" value="{{ old('name', $program->name) }}" required>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="code" class="form-control border-left-0 py-2 @error('code') is-invalid @enderror" value="{{ old('code', $program->code) }}" required>
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                @php $selectedCampuses = $program->campuses->pluck('id')->toArray(); @endphp
                                <select name="campuses[]" class="form-control" data-choices data-choices-removeItem multiple>
                                    @foreach($campuses as $campus)
                                        <option value="{{ $campus->id }}" {{ in_array($campus->id, old('campuses', $selectedCampuses)) ? 'selected' : '' }}>{{ $campus->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="status" class="form-select py-2 @error('status') is-invalid @enderror" required>
                                    <option value="draft" {{ old('status', $program->status) == 'draft' ? 'selected' : '' }}>{{ __('educational::messages.status_draft') }}</option>
                                    <option value="published" {{ old('status', $program->status) == 'published' ? 'selected' : '' }}>{{ __('educational::messages.status_published') }}</option>
                                    <option value="running" {{ old('status', $program->status) == 'running' ? 'selected' : '' }}>{{ __('educational::messages.status_running') }}</option>
                                    <option value="completed" {{ old('status', $program->status) == 'completed' ? 'selected' : '' }}>{{ __('educational::messages.status_completed') }}</option>
                                    <option value="archived" {{ old('status', $program->status) == 'archived' ? 'selected' : '' }}>{{ __('educational::messages.status_archived') }}</option>
                                </select>
                        ...
                        <div class="col-12 mt-4 pt-2">
                            <div class="d-flex align-items-center mb-1">
                                <i class="ri-calendar-todo-line fs-18 text-warning me-2"></i>
                                <h6 class="mb-0 fw-bold border-bottom border-warning border-2 pb-1">الفترة الزمنية</h6>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="date" name="starts_at" class="form-control border-left-0 py-2 @error('starts_at') is-invalid @enderror" value="{{ old('starts_at', $program->starts_at ? \Carbon\Carbon::parse($program->starts_at)->format('Y-m-d') : '') }}">
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="date" name="ends_at" class="form-control border-left-0 py-2 @error('ends_at') is-invalid @enderror" value="{{ old('ends_at', $program->ends_at ? \Carbon\Carbon::parse($program->ends_at)->format('Y-m-d') : '') }}">
                                </div>
