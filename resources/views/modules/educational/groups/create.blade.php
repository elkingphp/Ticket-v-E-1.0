...
@section('title', __('educational::messages.add_group'))
    ...
    <div class="col-xl-10">
        <div class="card border-0 shadow-sm overflow-hidden">
            <div class="card-header bg-soft-info border-0 py-3">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1">
                        <h5 class="card-title mb-0 fw-bold">{{ __('educational::messages.add_group') }}</h5>
                        <p class="text-muted mb-0 small">إنشاء مجموعة تدريبية جديدة وربطها بالبرنامج والتخصص المناسب.</p>
                    </div>
            ...
            <div class="card-body p-4">
                <form action="{{ route('educational.groups.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="name" class="form-control py-2 @error('name') is-invalid @enderror" placeholder="مثال: مجموعة أ - الدفعة الأولى" value="{{ old('name') }}" required>
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="term" class="form-control py-2 @error('term') is-invalid @enderror" placeholder="مثال: 2025-Q1" value="{{ old('term') }}">
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="capacity" class="form-control py-2 @error('capacity') is-invalid @enderror" value="{{ old('capacity', 20) }}" min="1" required>
                                </div>
                                    ...
                                    @foreach($tracks as $track)
                                        <optgroup label="{{ $track->name }}">
                                            @foreach($track->jobProfiles as $jp)
                                                <option value="{{ $jp->id }}" {{ old('job_profile_id') == $jp->id ? 'selected' : '' }}>{{ $jp->name }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <select name="status" id="group_status" class="form-select py-2 @error('status') is-invalid @enderror" required>
                                    ...
                                    <option value="completed" {{ old('status') == 'completed' ? 'selected' : '' }}>{{ __('educational::messages.status_completed') ?? 'مكتملة' }}</option>
                                    <option value="cancelled" {{ old('status') == 'cancelled' ? 'selected' : '' }}>{{ __('educational::messages.status_cancelled') }}</option>
                                    <option value="transferred" {{ old('status') == 'transferred' ? 'selected' : '' }}>{{ __('educational::messages.status_transferred') }}</option>
                                </select>
                        ...
                        <div class="col-md-12" id="cancellation_reason_container" style="display: none;">
                            <div class="form-group mb-0">
                                ...
                                <textarea name="cancellation_reason" class="form-control py-2 @error('cancellation_reason') is-invalid @enderror" rows="3" placeholder="توضيح سبب الإلغاء...">{{ old('cancellation_reason') }}</textarea>
                            ...
                            <div class="form-group mb-0">
                                ...
                                <select name="transferred_to_group_id" class="form-select py-2 @error('transferred_to_group_id') is-invalid @enderror">
                                    ...
                                    @foreach($allGroups as $g)
                                        <option value="{{ $g->id }}" {{ old('transferred_to_group_id') == $g->id ? 'selected' : '' }}>{{ $g->name }}</option>
                                    @endforeach
                                </select>
