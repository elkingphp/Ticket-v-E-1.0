<header id="page-topbar">
    <div class="layout-width">
        <div class="navbar-header">
            <div class="d-flex">
                <!-- LOGO -->
                <!-- Logo removed from topbar - handled by sidebar -->
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="dropdown ms-1 topbar-head-dropdown header-item">
                    <button type="button" class="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle"
                        ...
                        @if (app()->getLocale() == 'ar')
                            <img src="{{ asset('assets/images/flags/sa.svg') }}" alt="Arabic" height="20"
                                class="rounded">
                        @else
                            <img src="{{ asset('assets/images/flags/us.svg') }}" alt="English" height="20"
                                class="rounded">
                        @endif
                    </button>
                    <div class="dropdown-menu dropdown-menu-end">
                        <a href="{{ route('lang.switch', 'en') }}"
                            class="dropdown-item notify-item language py-2 @if (app()->getLocale() == 'en') active @endif"
                        ...
                        <a href="{{ route('lang.switch', 'ar') }}"
                            class="dropdown-item notify-item language py-2 @if (app()->getLocale() == 'ar') active @endif"
                            data-lang="ar" title="Arabic">
                            <img src="{{ asset('assets/images/flags/sa.svg') }}" alt="user-image" class="me-2 rounded"
                                height="18">
                            <span class="align-middle">{{ __('core::messages.lang_arabic') }}</span>
                        </a>
                    </div>
                </div>
                ...
                <div class="dropdown ms-1 topbar-head-dropdown header-item" id="notificationDropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0"
                        ...
                        <div class="dropdown-head bg-primary bg-pattern rounded-top">
                            <div class="p-3">
                                <div class="row align-items-center">
                                    <div class="col">
                                        <h6 class="m-0 fs-16 fw-semibold text-white">{{ __('sidebar.notifications') }}
                                        </h6>
                                    </div>
                                    <div class="col-auto">
                                        @if ($unreadNotificationsCount > 0)
                        ...
                        <div class="px-2 pt-2 border-bottom border-bottom-dashed">
                            <ul class="nav nav-tabs dropdown-tabs nav-tabs-custom flex-nowrap overflow-auto"
                                role="tablist" style="scrollbar-width: none; -ms-overflow-style: none;">
                                <li class="nav-item flex-shrink-0 waves-effect waves-light">
                                    <a class="nav-link active" data-bs-toggle="tab" href="#all-noti-tab" role="tab">
                                        {{ __('core::notifications.all') }}
                                        <span class="badge bg-soft-secondary text-secondary ms-1 fs-10"
                                            id="badge-all">{{ $unreadNotificationsCount }}</span>
                                    </a>
                                </li>
                                @foreach ($notificationCategories as $key => $category)
                                    <li class="nav-item flex-shrink-0 waves-effect waves-light">
                                        <a class="nav-link" data-bs-toggle="tab" href="#{{ $category['id'] }}"
                                            role="tab" data-module="{{ $key }}">
                                            {{ $category['name'] }}
                                            <span class="badge bg-soft-primary text-primary ms-1 fs-10"
                                                id="badge-{{ $category['id'] }}"
                                                style="{{ $category['unread'] == 0 ? 'display: none;' : '' }}">
                                                {{ $category['unread'] }}
                                            </span>
                                        </a>
                                    </li>
                            ...
                            <div class="tab-pane fade show active py-2" id="all-noti-tab" role="tabpanel">
                                <div data-simplebar style="max-height: 300px; overflow-x: hidden;" class="pe-2">
                                    @forelse($allNotifications as $notification)
                                        @include('core::layouts.partials.notification_item', [
                        ...
                        @if ($unreadNotificationsCount > 0 || $allNotifications->isNotEmpty())
                            <div class="my-2 py-2 text-center view-all-button border-top border-top-dashed">
                                <a href="{{ route('notifications.index') }}"
                                    class="btn btn-soft-primary btn-sm px-4">
                                    {{ __('core::notifications.view_all') }}
                                    <i
                                        class="ri-arrow-{{ app()->getLocale() == 'ar' ? 'left' : 'right' }}-line align-middle ms-1"></i>
                                </a>
                            </div>
                        @endif
                    </div>
                    ...
                    <button type="button" class="btn" id="page-header-user-dropdown" data-bs-toggle="dropdown"
                        ...
                        <span class="d-flex align-items-center">
                            ...
                            <span class="text-start ms-xl-2">
                                <span
                                    class="d-none d-xl-inline-block ms-1 fw-medium user-name-text">{{ auth()->user()?->full_name ?? 'Guest' }}</span>
                                <span
                                    class="d-none d-xl-block ms-1 fs-12 user-name-sub-text">{{ auth()->user()?->roles->first()?->display_name ?? (auth()->user()?->roles->first()?->name ?? 'User') }}</span>
                            </span>
                        </span>
                    </button>
                    <div class="dropdown-menu dropdown-menu-end">
                        <!-- item-->
                        <h6 class="dropdown-header">{{ __('messages.welcome') }}, {{ auth()->user()?->first_name }}!
                        </h6>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0">
            <div class="modal-header p-3 bg-primary-subtle">
                <h5 class="modal-title" id="notificationDetailModalLabel">{{ __('core::notifications.details') }}
                </h5>
            ...
            <div class="modal-footer border-top border-top-dashed">
                <button type="button" class="btn btn-light"
                    data-bs-dismiss="modal">{{ __('core::notifications.close') }}</button>
                <a href="#" id="modal-notification-action-btn"
                    class="btn btn-primary d-none">{{ __('core::notifications.view_details') }}</a>
            </div>
        </div>
    </div>
</div>
