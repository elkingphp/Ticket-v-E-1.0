                        ...
                        <div class="row g-3">
                                ...
                                <div class="search-box">
                                    <select class="form-control" name="event">
                                        ...
                                        @foreach ($events as $event)
                                            <option value="{{ $event }}"
                                                {{ request('event') == $event ? 'selected' : '' }}>{{ ucfirst($event) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-xxl-2 col-sm-6">
                                <select class="form-control" name="log_level">
                                    ...
                                    <option value="info" {{ request('log_level') == 'info' ? 'selected' : '' }}>
                                        {{ __('core::audit.info') }}</option>
                                    <option value="warning" {{ request('log_level') == 'warning' ? 'selected' : '' }}>
                                        {{ __('core::audit.warning') }}</option>
                                    <option value="critical" {{ request('log_level') == 'critical' ? 'selected' : '' }}>
                                        {{ __('core::audit.critical') }}</option>
                                </select>
                            </div>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="flex-grow-1 ms-2 name">
                                                    {{ $log->user ? $log->user->full_name : __('core::audit.system') }}
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span
                                                class="badge bg-{{ $log->event == 'created' ? 'success' : ($log->event == 'updated' ? 'info' : ($log->event == 'deleted' ? 'danger' : 'primary')) }}-subtle text-{{ $log->event == 'created' ? 'success' : ($log->event == 'updated' ? 'info' : ($log->event == 'deleted' ? 'danger' : 'primary')) }} text-uppercase">
                                                {{ __("core::profile.{$log->event}_activity") != "core::profile.{$log->event}_activity" ? __("core::profile.{$log->event}_activity") : $log->event }}
                                            </span>
                                        </td>
                                        <td>{{ $log->category }}</td>
                                        <td>
                                            <span
                                                class="badge {{ $log->log_level == 'critical' ? 'bg-danger' : ($log->log_level == 'warning' ? 'bg-warning' : 'bg-info') }}">
                                                {{ __('core::audit.' . strtolower($log->log_level)) }}
                                            </span>
                                        </td>
                                        <td>{{ $log->ip_address }}</td>
                                        <td>{{ $log->created_at->translatedFormat('d M, Y H:i') }}</td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-light show-log-details"
                                                data-id="{{ $log->id }}">
