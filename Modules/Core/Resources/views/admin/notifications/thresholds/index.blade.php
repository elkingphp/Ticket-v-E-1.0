...
@section('title', 'إدارة حدود الإشعارات')

@section('content')
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">إدارة حدود الإشعارات (Notification Thresholds)</h4>
            <div class="page-title-right">
                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createThresholdModal">
                    <i class="bx bx-plus"></i> إضافة حد جديد
                </button>
            </div>
        </div>
    </div>
                    ...
                    <table class="table table-hover align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>الحد الأقصى (Max Count)</th>
                                <th>النافذة الزمنية (Time Window)</th>
                                <th>الأهمية (Severity)</th>
                                <th>الحالة</th>
                                <th>الوصف</th>
                                <th>الإجراءات</th>
                            </tr>
                        ...
                        <tbody>
                            ...
                            <tr id="threshold-{{ $threshold->id }}">
                                ...
                                <td>{{ $threshold->max_count }} مرات</td>
                                <td>{{ $threshold->time_window }} ثانية</td>
                                <td>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-soft-info" onclick="editThreshold({{ $threshold->id }}, {{ json_encode($threshold) }})">
                                        <i class="bx bx-edit"></i>
                                    </button>
                                    <button class="btn btn-sm btn-soft-danger" onclick="deleteThreshold({{ $threshold->id }})">
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="text-center py-4 text-muted">لا توجد حدود معرفة.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="thresholdModalTitle">إضافة حد جديد</h5>
            ...
            <form id="thresholdForm" onsubmit="saveThreshold(event)">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="event_type" class="form-label">نوع الحدث (Event Type)</label>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="max_count" class="form-label">الحد الأقصى (Max Count)</label>
                            <input type="number" class="form-control" id="max_count" name="max_count" required min="1">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="time_window" class="form-label">النافذة الزمنية (ثواني)</label>
                            <input type="number" class="form-control" id="time_window" name="time_window" required min="60">
                        </div>
                    </div>
                    ...
                    <div class="form-check form-switch mb-3">
                        ...
                        <label class="form-check-label" for="enabled">مفعل</label>
                    </div>
                </div>
...
document.querySelector('[data-bs-target="#createThresholdModal"]').addEventListener('click', () => {
    ...
    document.getElementById('thresholdModalTitle').textContent = 'إضافة حد جديد';
    thresholdModal.show();
});
...
function editThreshold(id, data) {
    ...
    document.getElementById('thresholdModalTitle').textContent = 'تعديل الحد';
    thresholdModal.show();
}
...
function saveThreshold(event) {
    ...
    .then(result => {
        if (result.success) {
            ...
            alert('حدث خطأ: ' + (result.message || 'Unknown error'));
        }
    })
