                ...
                <div class="text-center">
                    <div class="profile-user position-relative d-inline-block mx-auto mb-4">
                        ...
                        @else
                            ...
                            <img src="" class="rounded-circle avatar-xl img-thumbnail user-profile-image shadow d-none" alt="user-profile-image" id="user-avatar-image">
                        @endif
                        <div class="avatar-xs p-0 rounded-circle profile-photo-edit">
                            <input id="profile-img-file-input" type="file" name="avatar" class="profile-img-file-input" accept="image/*">
                        ...
                        @if($user->avatar)
                            <button type="button" class="btn btn-danger btn-sm rounded-circle position-absolute top-0 end-0 shadow" onclick="deleteAvatar()" id="delete-avatar-btn" style="width: 28px; height: 28px; padding: 0;">
                                <i class="ri-delete-bin-line"></i>
                            </button>
                        @endif
                    </div>
                    <h5 class="fs-16 mb-1">{{ $user->full_name }}</h5>
                    <p class="text-muted mb-0">{{ $user->roles->first()->name ?? __('core::profile.user') }}</p>
                </div>
            </div>
        </div>
        <!--end card-->
            ...
            <div class="card-body">
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div class="flex-shrink-0">
                        <span class="badge {{ $user->security_status['class'] }} fs-12">{{ $user->security_status['label'] }}</span>
                    </div>
                </div>
                <div class="text-center py-2">
                    <div class="position-relative d-inline-block">
                        <svg class="progress-ring" width="120" height="120">
                            <circle class="progress-ring__circle-bg" stroke="#f3f3f3" stroke-width="8" fill="transparent" r="50" cx="60" cy="60"/>
                            <circle class="progress-ring__circle" stroke="{{ $user->security_status['color'] == 'success' ? '#0ab39c' : ($user->security_status['color'] == 'warning' ? '#f7b84b' : '#f06548') }}" stroke-width="8" stroke-dasharray="314.159" stroke-dashoffset="{{ 314.159 - (314.159 * $user->profile_completion_score / 100) }}" stroke-linecap="round" fill="transparent" r="50" cx="60" cy="60"/>
                        </svg>
            ...
            <div class="card-body p-4">
                <div class="tab-content">
                    <div class="tab-pane active" id="personalDetails" role="tabpanel">
                        <form id="personal-details-form" action="{{ route('profile.update') }}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        <label for="firstnameInput" class="form-label">{{ __('core::profile.first_name') }}</label>
                                        <input type="text" class="form-control" name="first_name" id="firstnameInput" placeholder="{{ __('core::profile.enter_first_name') }}" value="{{ old('first_name', $user->first_name) }}">
                                    </div>
                                </div>
                                <!--end col-->
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        <label for="lastnameInput" class="form-label">{{ __('core::profile.last_name') }}</label>
                                        <input type="text" class="form-control" name="last_name" id="lastnameInput" placeholder="{{ __('core::profile.enter_last_name') }}" value="{{ old('last_name', $user->last_name) }}">
                                    </div>
                                </div>
                                <!--end col-->
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        <label for="phonenumberInput" class="form-label">{{ __('core::profile.phone_number') }}</label>
                                        <input type="text" class="form-control" name="phone" id="phonenumberInput" placeholder="{{ __('core::profile.enter_phone_number') }}" value="{{ old('phone', $user->phone) }}">
                                    </div>
                                </div>
                                <!--end col-->
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        <label for="emailInput" class="form-label">{{ __('core::profile.email_address') }}</label>
                                        <input type="email" class="form-control" name="email" id="emailInput" placeholder="{{ __('core::profile.enter_email') }}" value="{{ old('email', $user->email) }}">
                                    </div>
                                </div>
                                <!--end col-->
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        <label for="languageSelect" class="form-label">{{ __('core::profile.language') }}</label>
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        <label for="timezoneSelect" class="form-label">{{ __('core::profile.timezone') }}</label>
                                        <select class="form-control" name="timezone" id="timezoneSelect" data-choices>
                                ...
                                <!--end col-->
                                <div class="col-lg-12">
                                    <div class="hstack gap-2 justify-content-{{ app()->getLocale() == 'ar' ? 'start' : 'end' }}">
                                        <button type="submit" class="btn btn-primary">{{ __('core::profile.update') }}</button>
                                        <button type="button" class="btn btn-soft-secondary">{{ __('core::profile.cancel') }}</button>
                                    </div>
                                </div>
                                <!--end col-->
                            </div>
                    ...
                    <div class="tab-pane" id="changePassword" role="tabpanel">
                        <form id="change-password-form" action="{{ route('profile.password') }}" method="POST">
                            ...
                            <div class="row g-2">
                                <div class="col-lg-4">
                                    <div>
                                        <label for="oldpasswordInput" class="form-label">{{ __('core::profile.old_password') }}</label>
                                        <input type="password" class="form-control" name="current_password" id="oldpasswordInput" placeholder="{{ __('core::profile.enter_current_password') }}">
                                    </div>
                                </div>
                                <!--end col-->
                                <div class="col-lg-4">
                                    <div>
                                        <label for="newpasswordInput" class="form-label">{{ __('core::profile.new_password') }}</label>
                                        <input type="password" class="form-control" name="password" id="newpasswordInput" placeholder="{{ __('core::profile.enter_new_password') }}">
                                    </div>
                                </div>
                                <!--end col-->
                                <div class="col-lg-4">
                                    <div>
                                        <label for="confirmpasswordInput" class="form-label">{{ __('core::profile.confirm_password') }}</label>
                                        <input type="password" class="form-control" name="password_confirmation" id="confirmpasswordInput" placeholder="{{ __('core::profile.confirm_new_password') }}">
                                    </div>
                                </div>
                                <!--end col-->
                                <div class="col-lg-12">
                                    <div class="text-{{ app()->getLocale() == 'ar' ? 'start' : 'end' }}">
                                        <button type="submit" class="btn btn-success">{{ __('core::profile.change_password') }}</button>
                                    </div>
                                </div>
                                <!--end col-->
                            </div>
                    ...
                    <div class="tab-pane" id="twoFactor" role="tabpanel">
                        ...
                        <p class="text-info fs-12"><i class="ri-information-line me-1"></i> {{ __('core::profile.2fa_google_tip') }}</p>

                        @if(!auth()->user()->two_factor_secret)
                            <form id="enable-2fa-form" method="POST" action="{{ url('user/two-factor-authentication') }}">
                                @csrf
                        ...
                        @elseif(!auth()->user()->two_factor_confirmed_at)
                            ...
                            <div class="mb-4 text-center p-3 bg-light rounded">
                                ...
                                <p class="mt-3 text-muted">Setup Key: <code>{{ decrypt(auth()->user()->two_factor_secret) }}</code></p>
                            </div>

                            <form id="confirm-2fa-form" method="POST" action="{{ url('user/confirmed-two-factor-authentication') }}">
                                @csrf
                                <div class="row justify-content-center">
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            <label for="2fa_code" class="form-label">{{ __('core::profile.enter_2fa_code') }}</label>
                                            <input type="text" class="form-control text-center fs-18 fw-bold" name="code" id="2fa_code" maxlength="6" placeholder="000000" required autofocus autocomplete="one-time-code">
                                        </div>
                        ...
                        @else
                            ...
                            <div class="mt-4">
                                ...
                                <form id="regenerate-codes-form" method="POST" action="{{ url('user/two-factor-recovery-codes') }}">
                                    @csrf
                                    <button type="submit" class="btn btn-soft-primary">{{ __('core::profile.regenerate_codes') }}</button>
                                </form>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="tab-pane" id="sessions" role="tabpanel">
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <div class="flex-shrink-0">
                                <button type="button" class="btn btn-soft-danger btn-sm" onclick="terminateOtherSessions()">
                                    <i class="ri-logout-box-line align-bottom me-1"></i> {{ __('core::profile.terminate_others') }}
                                </button>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    @foreach($sessions as $session)
                                        <tr id="session-{{ $session['id'] }}">
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <div class="avatar-xs flex-shrink-0 me-3">
                                                        <div class="avatar-title bg-soft-primary text-primary rounded-circle fs-16">
                                                            <i class="{{ $session['agent']['is_mobile'] ? 'ri-smartphone-line' : 'ri-computer-line' }}"></i>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <h6 class="fs-14 mb-1">{{ $session['agent']['browser'] }} on {{ $session['agent']['platform'] }}</h6>
                                                        <p class="text-muted mb-0 fs-12">{{ $session['agent']['device'] ?: __('core::profile.unknown_device') }}</p>
                                                    </div>
                                                    @if($session['is_current_device'])
                                                        <span class="badge bg-success-subtle text-success ms-2">{{ __('core::profile.current') }}</span>
                                                    @endif
                                                </div>
                                            </td>
                                            ...
                                            <td class="text-end">
                                                @if(!$session['is_current_device'])
                                                    <button type="button" class="btn btn-icon btn-soft-danger btn-sm" onclick="terminateSession('{{ $session['id'] }}')">
                        ...
                        <div class="profile-timeline">
                            <div class="acitivity-timeline" id="activity-timeline">
                                @if($activities->count() > 0)
                                    @include('core::profile.partials.activity_items', ['activities' => $activities])
                                @else
                            ...
                            @if($activities->hasMorePages())
                                <div class="text-center mt-4">
                                    <button type="button" class="btn btn-soft-primary btn-sm" id="load-more-activities" data-next-page="{{ $activities->currentPage() + 1 }}">
                        ...
                        <form id="notification-preferences-form">
                            ...
                            <div class="table-responsive">
                                <table class="table table-nowrap align-middle mb-0">
                                    <thead class="table-light">
                                        <tr>
                                            <th scope="col" style="width: 40%;">{{ __('core::profile.event_description') }}</th>
                                            <th scope="col" class="text-center">{{ __('core::profile.in_app') }}</th>
                                            <th scope="col" class="text-center">{{ __('core::profile.email') }}</th>
                                            <th scope="col" class="text-center">{{ __('core::profile.browser') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                                        ...
                                                        <div class="flex-grow-1">
                                                            <h6 class="fs-14 mb-1">
                                                                ...
                                                                @if($type->is_mandatory)
                                                                    <span class="badge bg-danger-subtle text-danger ms-1 fs-10">{{ __('core::profile.mandatory') }}</span>
                                                                @endif
                                                            </h6>
                                                            <p class="text-muted mb-0 fs-12 text-wrap">{{ __('core::profile.events.' . $type->key . '.description') }}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                        ...
                                                        <div class="form-check form-switch form-switch-md d-inline-block">
                                                            <input class="form-check-input notification-channel-toggle" 
                                                                   ...
                                                                   @if(in_array($channel, $type->available_channels))
                                                                       @if(in_array($channel, $enabledChannels) || ($type->is_mandatory && $channel === 'database')) checked @endif
                                                                       @if($type->is_mandatory && $channel === 'database') disabled @endif
                                                                   @else
                                                                       disabled
                                                                   @endif
                                                                   data-event="{{ $type->key }}"
                    ...
                    <div class="tab-pane" id="privacy" role="tabpanel">
                        <div class="mb-4">
                            ...
                            <button type="button" class="btn btn-soft-primary" onclick="requestDataExport()">
                                <i class="ri-download-2-line align-bottom me-1"></i> {{ __('core::profile.request_data_export') }}
                            </button>
                        </div>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            ...
                                                            <div>
                                                                ...
                                                                <small class="text-muted">{{ $export['created_at'] }}</small>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>{{ $export['size'] }}</td>
                                                    <td class="text-end">
                                                        <div class="hstack gap-2 justify-content-end">
                                                            <a href="{{ $export['url'] }}" class="btn btn-sm btn-soft-info">
                                                                <i class="ri-download-line font-size-14 align-middle me-1"></i> {{ __('core::profile.download') }}
                                                            </a>
                                                            <button type="button" class="btn btn-sm btn-soft-danger" onclick="deleteExport('{{ $export['name'] }}')">
                                                                <i class="ri-delete-bin-line font-size-14 align-middle me-1"></i> {{ __('core::profile.delete') }}
                                                            </button>
                                                        </div>
                                                    </td>
                        ...
                        <div class="mb-4">
                            ...
                            @if($user->scheduled_for_deletion_at)
                                <div class="alert alert-warning border-0" role="alert">
                                    <strong>{{ __('core::profile.scheduled_for_deletion_on') }}:</strong> {{ $user->scheduled_for_deletion_at->format('M d, Y') }}
                                    <button type="button" class="btn btn-sm btn-link text-warning p-0 ms-2" onclick="cancelAccountDeletion()">{{ __('core::profile.cancel_deletion') }}</button>
                                </div>
                            @else
...
<script>
        ...
        if (password) {
            try {
                ...
                if (data.success) {
                    ...
                    Swal.fire('{{ __('core::profile.error') }}', data.message || '{{ __('core::profile.incorrect_password') }}', 'error');
                    return false;
                }
            } catch (error) {
                Swal.fire('{{ __('core::profile.error') }}', '{{ __('core::profile.verify_failed') }}', 'error');
                return false;
            }
        }
        return false;
    ...
    async function handleFormSubmit(e) {
        ...
        const submitAction = async () => {
            ...
            submitBtn.innerHTML = '<i class="bx bx-loader bx-spin font-size-16 align-middle me-2"></i> {{ __('core::profile.update') }}...';
            ...
            try {
                ...
                if (response.status === 403) {
                    ...
                    if (response.status === 422) {
                        ...
                        Swal.fire('Error', data.message || 'Server error occurred (' + response.status + ')', 'error');
                    }
                } else {
                    if (data.success || response.status === 200 || response.status === 204) {
                        Toast.fire({ icon: 'success', title: data.message || '{{ __('core::profile.update_success') }}' });
                        if (data.redirect) {
                            window.location.href = data.redirect;
                        } else if (form.id === 'personal-details-form' || form.id === 'enable-2fa-form' || form.id === 'confirm-2fa-form' || form.id === 'disable-2fa-form' || form.id === 'regenerate-codes-form') {
    ...
    ['personal-details-form', 'change-password-form', 'enable-2fa-form', 'confirm-2fa-form', 'disable-2fa-form', 'regenerate-codes-form'].forEach(id => {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (!document.getElementById('delete-avatar-btn')) {
                    const deleteBtnHtml = `<button type="button" class="btn btn-danger btn-sm rounded-circle position-absolute top-0 end-0 shadow" onclick="deleteAvatar()" id="delete-avatar-btn" style="width: 28px; height: 28px; padding: 0;"><i class="ri-delete-bin-line"></i></button>`;
                    document.querySelector('.profile-user').insertAdjacentHTML('beforeend', deleteBtnHtml);
                }
            } else {
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (data.success) {
                        ...
                        const headerAvatar = document.querySelector('.header-profile-user'); // Adjust selector as needed
                        if (headerAvatar) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (response.status === 405) {
                        ...
                        Swal.fire('Error', data.message || 'Server error (' + response.status + ')', 'error');
                    }
                }
            } catch (err) {
    ...
    function saveNotificationPreferences() {
        handleFormSubmit({ target: document.getElementById('notification-preferences-form'), preventDefault: () => {} });
    }
